#!/bin/bash --login
# ============================================================================
# WMT26 QE baseline — Qwen3.6-35B-A3B, all variants, all language pairs.
#
...
#   Variant 0: quality_estimation_outputs_qwen36_no_ref
#   Variant 1: quality_estimation_outputs_qwen36_ref
#   Variant 2: quality_estimation_outputs_qwen36_thinking_no_ref
#   Variant 3: quality_estimation_outputs_qwen36_thinking_ref
#
# GPU memory (BF16 weights ~70 GB):
#   2× B200 (140 GB SXM) — use --gres=gpu:b200:2 and --tensor-parallel-size 2
#   4× A100 80GB          — change --gres=gpu:4 and --tensor-parallel-size 4
#
...
#
# Submit all 108 jobs:
#   sbatch slurm/run_qwen36.sh
# Smoke-test a single job (cs-de, no-thinking, no-ref):
#   sbatch --array=0 slurm/run_qwen36.sh
# Run only no-thinking variants (IDs 0-53):
#   sbatch --array=0-53 slurm/run_qwen36.sh
# Run only thinking variants (IDs 54-107):
#   sbatch --array=54-107 slurm/run_qwen36.sh
# Run only no-ref variants (IDs 0-26 and 54-80):
#   sbatch --array=0-26,54-80 slurm/run_qwen36.sh
#
# After all jobs complete, merge each variant into one file (source order):
#   python merge_shards.py --model qwen36 \
#       --output-dir quality_estimation_outputs_qwen36_no_ref
#   python merge_shards.py --model qwen36 \
#       --output-dir quality_estimation_outputs_qwen36_ref
#   python merge_shards.py --model qwen36_thinking \
#       --output-dir quality_estimation_outputs_qwen36_thinking_no_ref
#   python merge_shards.py --model qwen36_thinking \
#       --output-dir quality_estimation_outputs_qwen36_thinking_ref
# ============================================================================

#SBATCH --time=48:00:00
#SBATCH --job-name=qe_qwen36
#SBATCH --nodes=1
...
case "$VARIANT" in
    0) THINKING=false; WITH_REF=false; OUTPUT_DIR="quality_estimation_outputs_qwen36_no_ref"          ;;
    1) THINKING=false; WITH_REF=true;  OUTPUT_DIR="quality_estimation_outputs_qwen36_ref"              ;;
    2) THINKING=true;  WITH_REF=false; OUTPUT_DIR="quality_estimation_outputs_qwen36_thinking_no_ref"  ;;
    3) THINKING=true;  WITH_REF=true;  OUTPUT_DIR="quality_estimation_outputs_qwen36_thinking_ref"     ;;
    *)
...
echo "=== Job ${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}: model=qwen36 pair=${PAIR} variant=${VARIANT} thinking=${THINKING} ref=${WITH_REF} ==="
nvidia-smi
